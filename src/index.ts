...
import {
  ...
  function parseDeadlineInput(input: string): string | null {
    ...
    const relativeMatch = trimmed.match(
      /^(今日|明日|明後日|(\d+)日後)(?:の?(\d{1,2})時(?:(\d{1,2})分)?)?$/
    );
    if (relativeMatch) {
      let daysToAdd = 0;
      if (relativeMatch[1] === "今日") daysToAdd = 0;
      else if (relativeMatch[1] === "明日") daysToAdd = 1;
      else if (relativeMatch[1] === "明後日") daysToAdd = 2;
      else if (relativeMatch[2] != null) daysToAdd = parseInt(relativeMatch[2], 10);
      else return null;
      const hour =
        relativeMatch[3] != null ? parseInt(relativeMatch[3], 10) : null;
      const minute =
        relativeMatch[4] != null ? parseInt(relativeMatch[4], 10) : null;
  ...
  const textEventHandler = async (
    ...
    if (state.type === "task_add") {
      ...
      if (state.step === "priority") {
        ...
        await reply(
          "いつまでに終わらせたい？（例: 明日 / 3日後 / 12月25日14時30分）"
        );
        return undefined;
      }
      if (state.step === "deadline") {
        ...
        if (deadlineStr === null) {
          await reply(
            "日付がわかりません。例: 今日 / 明日 / 3日後の18時 / 12月25日 のように入力してください。"
          );
          return undefined;
        }
        const task: Task = {
